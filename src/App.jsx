...
import BmiCalculator from './BmiCalculator'; // 引入剛剛新增的 BMI 元件

function App() {
    // 新增：用來控制目前顯示哪一個工具 ('converter' 或 'bmi')
    ...
    useEffect(() => {
        if (appMode === 'converter' && category === CATEGORIES.CURRENCY && Object.keys(rates).length === 0) {
            fetchRates();
        }
    }, [category, appMode]);
    ...
    return (
        <div className="app-container">
            <div className="header">
                <h1>{appMode === 'converter' ? 'Unit Converter' : 'BMI Calculator'}</h1>
            </div>

            {/* 新增：主功能切換導覽列 */}
            <div className="tabs" style={{ marginBottom: '1.5rem' }}>
                <div
                    className={`tab ${appMode === 'converter' ? 'active' : ''}`}
                ...
                <div
                    className={`tab ${appMode === 'bmi' ? 'active' : ''}`}
                    onClick={() => setAppMode('bmi')}
                    style={{ flex: 1, textAlign: 'center' }}
                >
                    BMI 計算
                </div>
